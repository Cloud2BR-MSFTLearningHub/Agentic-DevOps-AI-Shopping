...
def create_app() -> FastAPI:
    ...
    if config.a2a.mode in [ServerMode.A2A, ServerMode.HYBRID]:
        logger.info("Setting up enhanced A2A protocol endpoints with advanced UX features")
        
        # Initialize enhanced A2A routers
        enhanced_chat_router = EnhancedA2AChatRouter(
        ...
        # Add enhanced A2A routers
        app.include_router(enhanced_chat_router.router)
        app.include_router(server_router.get_router())
        
        # Store references for lifecycle management
        app.state.enhanced_chat_router = enhanced_chat_router
